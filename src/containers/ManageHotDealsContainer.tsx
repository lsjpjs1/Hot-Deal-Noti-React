...
const ManageHotDealsContainer = () => {
  ...
  const hotDeals = useSelector(
    (state: RootState) => state.hotDealReducer.hotDeals
  );
  const totalPages = useSelector(
    (state: RootState) => state.hotDealReducer.totalPages
  );
  const getHotDealRequest = useSelector(
    (state: RootState) => state.hotDealReducer.getHotDealRequest
  );
  const initData = useSelector(
    (state: RootState) => state.hotDealReducer.initData
  );
      ...
      >
        ...
        {initData != null && (
          <div>
            ...
            <Typography>
              {"최근 업데이트: " +
                moment(initData.recentUpdateTime, "YYYYMMDDHHmmss z")
                  .add(9, "h")
        ...
        <div>
          ...
          <HotDealSortingSelect
            onSelect={onHotDealSortingSelect}
          ></HotDealSortingSelect>
          <Container maxWidth={"sm"}>
            <ProductPurposeSelect
              onSelect={onProductPurposeSelect}
            ></ProductPurposeSelect>
            <ManufacturerSelect
              onSelect={onManufacturerSelect}
            ></ManufacturerSelect>
            <SourceSiteCheckBoxGroup
              onCheckBoxClick={onCheckBoxClick}
            ></SourceSiteCheckBoxGroup>
          </Container>
        </div>
        <ManageHotDealListView
          hotDeals={hotDeals}
          hotDealLinkOnClick={hotDealLinkOnClick}
        ></ManageHotDealListView>
        <PageView
          currentPage={getHotDealRequest.pageRequest.page}
          onPageChange={onPageChange}
          totalPageCount={totalPages}
        ></PageView>
        <p>
          <b>Email : whendiscount@gmail.com</b>
          <br />
...
export default ManageHotDealsContainer;
