...
const SearchBar = (searchBarProps: SearchBarProps) => {
  ...
  const onSliderChange = (valueList: any) => {
    // @ts-ignore
    mixpanel.track("discountFilterChange", {
      minDiscountRate: valueList[0],
      maxDiscountRate: valueList[1],
    });
    // @ts-ignore
  ...
  const onCheckBoxClick = (checked: boolean, sourceSite: string) => {
    if (checked) {
      mixpanel.track("sourceSiteSelect", {
  ...
  return (
    <S.SearchWrapper>
      <Autocomplete
        ...
        // @ts-ignore
        getOptionLabel={(option: ProductDto | string) =>
          typeof option == "object" ? option.fullModelName : option
        }
        // @ts-ignore
        onChange={(event, value: ProductDto) => {
          mixpanel.track("showProductPriceHistoryInSearch", {
      ...
      <IconButton
        ...
        onClick={() => {
          mixpanel.track("filterClick");
          setIsOpenFilter(true);
        }}
      >
        <S.HeaderMenuIcon src={"/image/icon/filter.png"} />
        <Typography style={{ color: "#4D4D4D", marginLeft: "6px" }}>
          필터
        </Typography>
      </IconButton>
